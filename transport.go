...
// chatPort is the single UDP port every room uses. Rooms are separated by
// their multicast group address (below) and, decisively, by encryption — so a
// fixed port keeps firewall rules simple ("allow UDP 48710") without letting
// rooms bleed into each other.
const chatPort = 48710

// transport is the peer-to-peer datagram layer. There is no server: every
// instance sends to, and listens on, the same multicast group. A copy of each
// datagram is also sent to the subnet broadcast address, which some networks
// deliver even when they silently drop multicast; duplicates are removed later
// by the (id, seq) dedup.
...
// groupFor maps a room name to a deterministic address in the
// 239.255.0.0/16 organization-local multicast scope.
func groupFor(room string) net.IP {
	h := sha256.Sum256([]byte("tchat-v2|group|" + room))
	return net.IPv4(239, 255, h[0], h[1])
}

func openTransport(room, ifaceName string, useBcast bool) (*transport, error) {
